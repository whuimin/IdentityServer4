...
namespace IdentityServer.IntegrationTests.Clients
{
    public class ClientCredentialsClient
    {
        ...
        public async Task Valid_request_multiple_audiences_should_return_expected_payload()
        {
            ...
            payload["aud"].ValueKind.Should().Be(JsonValueKind.Array);
            var audiences = payload["aud"].EnumerateArray().Select(a=>a.GetString());
        ...
        public async Task Valid_request_with_confirmation_should_return_expected_payload()
        {
            ...
            payload["cnf"].ValueKind.Should().Be(JsonValueKind.Object);
            var cnf = payload["cnf"].EnumerateObject().Where(x=>x.Name== "x5t#S256").FirstOrDefault();
            cnf.Value.ToString().Should().Be("foo");;
        }
